...
log = logging.getLogger("red.fluffy.streams.Streams")
...
class Streams(commands.Cog):
    ...
    async def trovo_alert(self, ctx: commands.Context, channel_name: str):
        """Toggle alerts in this channel for a Trovo stream."""
        await self.stream_alert(ctx, TrovoStream, channel_name.lower())
    ...
    async def stream_alert(self, ctx: commands.Context, _class, channel_name):
        ...
        if not stream:
            ...
            try:
                ...
                await ctx.send(
                    _(
                        "The Trovo API key is either invalid or has not been set. See "
                        "{command}."
                    ).format(command=f"`{ctx.clean_prefix}streamset trovokey`")
                )
                return
    ...
    async def role(self, ctx: commands.Context, *, role: discord.Role):
        ...
        if current_setting:
            ...
            await ctx.send(
                _("{role.mention} will no longer be mentioned for stream alerts.").format(
                    role=role
                )
            )
        else:
            ...
            msg = _(
                "When a stream or community is live, {role.mention} will be mentioned."
            ).format(role=role)
            # bruh fuck this dumb shit
    ...
    async def _get_mention_str(
        ...
        if can_mention_everyone:
            ...
            elif await settings.mention_here():
                mentions.append("@here")
        for role in guild.roles:
            if (can_mention_everyone or role.mentionable) and await self.config.role(
                role
            ).mention():
